...
if (isset($_POST['whosfollowers']))
{
    ...
    $addedusers = addfollows($c, $whosfollowers, $cursor, $count, $area, $sex, $haspic,
        $followers, $friends, $blogs);
...
function addfollows($c, $whosfollowers, $cursor, $count, $area, $sex, $haspic, $followers,
    $friends, $blogs)
{
    ...
    foreach ($c_followers['users'] as $item)
    {
        ...
        if ($relation['source']['following'] == true && $relation['target']['followed_by'] == true)
        { //是否已经关注
            continue;
        }
        ...
        if ($area != "" && $area != "不限")
        {
            if (strstr($item['location'], $area) == null)
            { //不符合地区要求
                continue;
            }
        }

        if ($haspic == "yes" && $item['profile_image_url'] == "")
        { //没有图像
            continue;
        }
